...
function add_table ($db,$tablename,$sortkey) {
   ...
   if (!$isbad && $tablename) {
      ...
      if ($r) {
         ...
         if ($r->fields["id"])
            $shortname.="$id";
  	 $r=$db->Execute("INSERT INTO tableoftables (id,sortkey,tablename,real_tablename,shortname,display,permission,table_desc_name) Values($id,'$sortkey','$tablename','$real_tablename','$shortname','Y','Users','$desc')");
