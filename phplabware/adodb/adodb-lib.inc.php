<?php
/* 
V1.63 13 Dec 2001 (c) 2000, 2001 John Lim (jlim@natsoft.com.my). All rights reserved.
...
function _adodb_getupdatesql(&$zthis,&$rs, $arrFields,$forceUpdate=false)
{
		if (!$rs) {
		...
		for ($i=0, $max=$rs->FieldCount(); $i < $max; $i++) {
			...
			if (isset($arrFields[$field->name])) {
				...
				if ($forceUpdate || strcmp($rs->fields[$i], $arrFields[$field->name])) {
					...
					// "mike" <mike@partner2partner.com> patch
					//PostgreSQL uses a 't' or 'f' and therefore needs to be processed as a string ('C') type field.
					if (($mt == "L") && (substr($zthis->databaseType,0,8) == "postgres")) 
						$mt = "C";
					
					switch($mt) {
						case "C":
						case "X":
							$updateSQL .= $field->name . " = " . $zthis->qstr($arrFields[$field->name]) . ", ";
							break;
						case "D":
...
function _adodb_getinsertsql(&$zthis,&$rs,$arrFields)
{
	if (!$rs) {
		...
		for ($i=0, $max=$rs->FieldCount(); $i < $max; $i++) {
			...
			if (isset($arrFields[$field->name])) {
				...
				switch($rs->MetaType($field->type)) {
					case "C":
					case "X":
						$values .= $zthis->qstr($arrFields[$field->name]) . ", ";
						break;
					case "D":
